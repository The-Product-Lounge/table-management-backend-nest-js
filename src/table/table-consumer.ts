...
export class TableConsumer {
  ...
  async joinTable(job: Job<UserWithIdDto>) {
    try {
      ...
      console.error(`Error While Joining Table - ${JSON.stringify(e.message)}, job:${JSON.stringify(job)}`);
  ...
  async queueCompleted(job: Job<UserWithIdDto>) {
    try {
      await job.remove();
      console.log(`Completed Handling Join Table Request - ${JSON.stringify(job.data)}`);
    } catch (e) {
      console.error(`Error While Joining Table - ${JSON.stringify(e.message)}, job:${JSON.stringify(job)}`);
      await job.moveToFailed(e.messag);
    }
